...
export default function AdminConfig() {
  ...
  const handleUpdate = async (key, newValue) => {
    ...
    setTimeout(() => setSavingKey(null), 1000) // Show a quick success state
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        <div className="bg-gray-900 p-6 rounded-xl border border-gray-800">
            ...
            <p className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-6">Changes update live for all active users.</p>
            
            <div className="space-y-4">
                {configs.map(item => (
                    <div key={item.id} className="bg-black border border-gray-800 p-4 rounded-xl">
                        ...
                        <div className="flex gap-2">
                            ...
                            <div className="w-20 flex items-center justify-center shrink-0">
                                {savingKey === item.config_key ? <span className="text-green-500 text-xs font-bold uppercase tracking-widest animate-pulse">Saved!</span> : null}
                            </div>
                        </div>
                    </div>
                ))}
