...
export default function FYP({ currentUser }) {
  ...
  const [viewMode, setViewMode] = useState('mixed') // 'mixed' or 'events'
  ...
  const onTouchEndHandler = () => {
    ...
    if (distance > 50) setViewMode('mixed') // Swipe left
    if (distance < -50) setViewMode('events') // Swipe right
  }
  ...
  const displayFeed = viewMode === 'events' ? feed.filter(item => item.type === 'event') : feed

  return (
    <div 
        className="max-w-2xl mx-auto p-4 animate-fade-in pb-32"
    ...
    >
      ...
      <div className="flex bg-gray-900 border border-gray-800 rounded-xl p-1 mb-8">
        <button 
          ...
          className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${viewMode === 'mixed' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'text-gray-500 hover:text-gray-300'}`}
        >
          All Activity
        </button>
        <button 
          onClick={() => setViewMode('events')}
          className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${viewMode === 'events' ? 'bg-purple-600/20 text-purple-400 border border-purple-500/30' : 'text-gray-500 hover:text-gray-300'}`}
        >
          Events Only
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center p-10 bg-[#090812]/80 border-2 border-gray-800 rounded-3xl">
          <p className="text-gray-500 font-bold tracking-widest uppercase text-sm">No updates in this view.</p>
        </div>
      ) : (
