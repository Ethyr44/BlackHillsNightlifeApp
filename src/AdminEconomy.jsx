...
export default function AdminEconomy() {
  ...
  const searchForUser = async () => {
      ...
      const { data } = await supabase.from('profiles').select('id, username, account_type, lifestyle_points, league_all_time, league_nightly').ilike('username', `%${searchUser}%`).limit(5)
      if (data) setUsers(data)
  }

  const saveUserPoints = async () => {
      await supabase.from('profiles').update({
          lifestyle_points: editLife, league_all_time: editLeagueAll, league_nightly: editLeagueNight
  ...
  const openEditor = (u) => {
      setEditingUser(u); setEditLife(u.lifestyle_points || 0); setEditLeagueAll(u.league_all_time || 0); setEditLeagueNight(u.league_nightly || 0);
  }
  ...
  const resetLeaderboard = async (columnName) => {
      if(window.confirm(`Are you SURE you want to reset the ${columnName} leaderboard to zero for everyone?`)) {
          // A quick trick to reset a column for all rows in Supabase using an update
        ...
        {editingUser ? (
            <div className="bg-black/50 p-4 rounded-xl border border-green-500/30">
                ...
                <div className="flex gap-2">
                    <button onClick={saveUserPoints} className="flex-1 bg-green-600 hover:bg-green-500 text-white py-2 rounded text-xs font-bold uppercase tracking-widest">Save Changes</button>
                    <button onClick={() => setEditingUser(null)} className="px-4 border border-gray-700 text-gray-400 hover:text-white rounded text-xs font-bold uppercase">Cancel</button>
                </div>
            </div>
        ) : (
                ...
                {users.map(u => (
                    <div key={u.id} className="flex justify-between items-center bg-black/40 p-3 rounded-lg border border-gray-800 hover:border-gray-600">
                        <div>
                            ...
                            <div className="text-xs text-gray-500">Life: {u.lifestyle_points||0} | League: {u.league_all_time||0}</div>
                        </div>
