...
export default function Live({ currentUser, onViewEntity }) {
    ...
    const [hostPinCode, setHostPinCode] = useState('')
    ...
    const verifyHostPin = () => {
        // 1117 for Admin unlock, 1144 for League Host unlock
        if (hostPinCode === '1144' || hostPinCode === '1117') {
            setHostMode(hostPinCode === '1144' ? 'league' : 'casual')
            setShowHostPin(false)
            setHostPinCode('')
        } else {
            alert('Incorrect PIN')
            setHostPinCode('')
                        ...
                        {joinedSessionId ? (
                            ...
                            <>
                                ...
                                <div className="mt-8 border-t border-gray-800 pt-8">
                                    <h3 className="text-gray-500 text-xs font-bold uppercase tracking-widest text-center mb-4">Host Controls</h3>
                                    {currentUser?.account_type !== 'Regular' ? (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <button onClick={() => setHostMode('casual')} className="bg-[#090812] border border-[#b347ff]/30 hover:border-[#b347ff] p-6 rounded-3xl flex flex-col items-center justify-center text-center transition-all group hover:shadow-[0_0_30px_rgba(179,71,255,0.2)]">
                                                ...
                                                <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Standard Queue</p>
                                            </button>
                                            <button onClick={() => setHostMode('league')} className="bg-[#090812] border border-[#00f5ff]/30 hover:border-[#00f5ff] p-6 rounded-3xl flex flex-col items-center justify-center text-center transition-all group hover:shadow-[0_0_30px_rgba(0,245,255,0.2)]">
                                                <div className="text-5xl mb-3 group-hover:scale-110 transition-transform">🏆</div>
                                                <h3 className="text-2xl font-['Bebas_Neue'] text-[#00f5ff] tracking-widest mb-1">League Mode</h3>
                                                <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Tournament Rules</p>
                                            </button>
                                        </div>
                                    ) : (
                                        <button onClick={() => setShowHostPin(true)} className="w-full max-w-sm mx-auto block bg-gray-900 border border-gray-800 hover:border-gray-600 text-gray-400 py-4 rounded-xl text-xs font-bold uppercase tracking-widest transition-colors">
                                            🔒 Unlock Host Access
                                        </button>
                                    )}
                                </div>
            ...
            {showHostPin && (
                <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-gray-900 border-2 border-[#00f5ff] p-8 rounded-3xl w-full max-w-sm text-center shadow-[0_0_50px_rgba(0,245,255,0.2)] animate-slide-up-fast">
                        ...
                        <h3 className="text-3xl font-['Bebas_Neue'] tracking-widest mb-2 text-[#00f5ff]">Host Access</h3>
                        <p className="text-gray-400 text-xs mb-6 uppercase tracking-widest">Enter authorization PIN.</p>
                        <input 
                            type="password" 
                            value={hostPinCode} 
                            onChange={e => setHostPinCode(e.target.value)} 
                            placeholder="****" 
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => {setShowHostPin(false); setHostPinCode('');}} className="flex-1 border border-gray-700 text-gray-400 rounded-xl text-xs font-bold uppercase tracking-widest hover:text-white py-3">Cancel</button>
                            <button onClick={verifyHostPin} className="flex-1 bg-[#00f5ff] text-black rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-[#00d5dd] shadow-[0_0_15px_rgba(0,245,255,0.4)] py-3">Verify</button>
                        </div>
                    </div>
                </div>
            )}
